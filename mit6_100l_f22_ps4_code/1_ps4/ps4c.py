...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    for pad in pads:
        ...
        score = sum([int(is_word(word_list, word)) for word in words])

        if score >= highest_score[1]:
            highest_score = (plaintext, score)
...
def decode_story():
    ...
    """
    return decrypt_message_try_pads(
        ps4b.EncryptedMessage(get_story_string()), get_story_pads()
    )
